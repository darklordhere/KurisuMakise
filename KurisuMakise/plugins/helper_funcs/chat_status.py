...
from KurisuMakise import (DEL_CMDS, RINTAROU, MAKISE_OWN, SUPPORT_CHAT, MAYURI,
                          FARIS, SUZUHA, dispatcher, REDIS)
...
def is_whitelist_plus(chat: Chat,
    ...
    return any(user_id in user
               for user in [SUZUHA, FARIS, MAYURI, MAKISE_OWN, RINTAROU])


def is_support_plus(chat: Chat,
                    user_id: int,
                    member: ChatMember = None) -> bool:
    return user_id in MAYURI or user_id in MAKISE_OWN or user_id in RINTAROU


def is_sudo_plus(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    return user_id in MAKISE_OWN or user_id in RINTAROU


def is_user_admin(chat: Chat, user_id: int, member: ChatMember = None) -> bool:
    if (chat.type == 'private' or user_id in MAKISE_OWN or user_id in RINTAROU or
...
def is_user_ban_protected(chat: Chat,
                          ...
                          member: ChatMember = None) -> bool:
    if (chat.type == 'private' or user_id in MAKISE_OWN or user_id in RINTAROU or
            user_id in SUZUHA or user_id in FARIS or
            chat.all_members_are_administrators or
            user_id in [777000, 1087968824
                       ]):  # Count telegram and Group Anonymous as admin
        return True
...
def user_can_ban(func):
    ...
    def user_is_banhammer(update: Update, context: CallbackContext, *args,
        ...
        if not (member.can_restrict_members or member.status == "creator"
               ) and not user in MAKISE_OWN and user not in [777000, 1087968824]:
            update.effective_message.reply_text(
...
from KurisuMakise.plugins import connection
