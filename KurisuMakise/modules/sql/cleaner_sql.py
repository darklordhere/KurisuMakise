...
from KurisuMakise.modules.sql import BASE, SESSION
...
def __load_cleaner_list():
    ...
    try:
        GLOBAL_IGNORE_COMMANDS = {
            str(x.command) for x in SESSION.query(CleanerBlueTextGlobal).all() # int(x.command) for x in SESSION.query(CleanerBlueTextGlobal).all()
        }
    finally:
        SESSION.close()
