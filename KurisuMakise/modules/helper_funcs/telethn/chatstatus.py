from KurisuMakise.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from KurisuMakise import DRAGONS
...
async def erza_is_admin(chat_id: int):
    status = False
    erza = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if erza.id == user.id:
