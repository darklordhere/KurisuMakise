...
from KurisuMakise import (DEV_USERS, LOGGER, OWNER_ID, DRAGONS, DEMONS, TIGERS,
                          WOLVES, dispatcher)
from KurisuMakise.modules.disable import DisableAbleCommandHandler
from KurisuMakise.modules.helper_funcs.chat_status import (
    bot_admin, can_restrict, connection_status, is_user_admin,
    is_user_ban_protected, is_user_in_chat, user_admin, user_can_ban)
from KurisuMakise.modules.helper_funcs.extraction import extract_user_and_text
from KurisuMakise.modules.helper_funcs.string_handling import extract_time
from KurisuMakise.modules.log_channel import gloggable, loggable
...
@user_can_ban
@loggable
def eject(update: Update, context: CallbackContext) -> str:
    chat = update.effective_chat
    user = update.effective_user
    message = update.effective_message
    log_message = ""
    ...
    if is_user_ban_protected(chat, user_id):
        message.reply_text("I really wish I could eject this user....")
        return log_message
    ...
    if res:
        ...
        bot.sendMessage(
            chat.id,
            f"Ejected! {mention_html(member.user.id, html.escape(member.user.first_name))}.",
            parse_mode=ParseMode.HTML)
        log = (
            f"<b>{html.escape(chat.title)}:</b>\n"
            f"#EJECTED\n"
        ...
        message.reply_text("Well damn, I can't eject that user.")
...
@bot_admin
@can_restrict
def ejectme(update: Update, context: CallbackContext):
    ...
    if res:
        update.effective_message.reply_text("*ejects you out of the group*")
    else:
...
__help__ = """
 • `/ejectme`*:* ejects the user who issued the command
...
dispatcher.add_handler(EJECT_HANDLER)
dispatcher.add_handler(UNBAN_HANDLER)
dispatcher.add_handler(ROAR_HANDLER)
dispatcher.add_handler(EJECTME_HANDLER)

__mod_name__ = "Bans"
__handlers__ = [
    BAN_HANDLER, TEMPBAN_HANDLER, EJECT_HANDLER, UNBAN_HANDLER, ROAR_HANDLER,
    EJECTME_HANDLER
]
