from KurisuMakise.plugins.helper_funcs.telethn import IMMUNE_USERS, telethn
from KurisuMakise import MAKISE_OWN
...
async def user_is_admin(user_id: int, message):
    ...
    async for user in telethn.iter_participants(
            message.chat_id, filter=ChannelParticipantsAdmins):
        if user_id == user.id or user_id in MAKISE_OWN:
            status = True
            break
    return status
